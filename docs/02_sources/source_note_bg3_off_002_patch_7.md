...
- 补丁说明明确写到：`Escape the Nautiloid` 任务在多人模式下如果有人在其他人还没醒来时先开启对话，可能会无法正常关闭；官方专门修复了这个问题。
...
- Patch 7 还修复了 tutorial 结束后的若干交接问题，包括 `Lae'zel` 不应继续保留 tutorial buff，以及如果 nautiloid 逃脱时带走了 `Shadowheart / Lae'zel` 的装备，她们在后续被招募时应获得新的装备。
...
- `推断`：连 `Escape the Nautiloid` 这种开场目标都以“可关闭 / 可能失效 / 需要修复”的方式出现，说明开场并不是纯展示段，而是正式任务流的一部分。
